...
enum MacroExecutor {
    ...
    private static func executeStep(
        ...
        switch kind {
        case .shell:
            try runProcess(executable: "/bin/zsh", arguments: ["-lc", value])
        case .openApplication:
            try runProcess(executable: "/usr/bin/open", arguments: ["-a", value])
        case .openURL:
            guard let url = URL(string: value), NSWorkspace.shared.open(url) else {
                throw macroError("Could not open URL: \(value)")
            }
        case .runShortcut:
            try runProcess(executable: "/usr/bin/shortcuts", arguments: ["run", value])
        case .runBlushMacro:
            try runNestedMacro(value: value, settings: settings, nestedSlots: nestedSlots, input: input)
        }
    }

    private static func runNestedMacro(
        value: String,
        settings: AppSettings,
        nestedSlots: [BlushSlot],
        input: DropInput?
    ) throws {
        guard let slot = BlushSlot(rawValue: value) else {
            throw macroError("Unknown macro: \(value)")
        }
        guard !nestedSlots.contains(slot) else {
            throw macroError("Nested macro cycle detected")
        }
        let nested = slot == .left ? settings.leftBlushMacro : settings.rightBlushMacro
        guard nested.isConfigured else {
            throw macroError("The selected macro is not configured")
        }
